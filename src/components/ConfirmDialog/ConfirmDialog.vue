...
<template>
  <v-dialog :max-width="props.width" v-model="dialog">
    <v-card class="pa-2">
      ...
      <v-card-actions>
        <v-btn @click="emit('close')" variant="text" >{{ $t('components.confirmDialog.cancel') }}</v-btn>
        <v-btn @click="emit('confirm')" variant="elevated"> {{$t('components.confirmDialog.delete')}} </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
